...
public class IceBlockManager : MonoBehaviour
{
	...
	public List<IceBlock> m_iceBlocks;
	...
	public void CheckForMatch(List<IceBlock> i_list)
	{
		...
		for (int i = 0; i < i_list.Count; i++)
		{
			...
			if(i_list[i].m_fish.m_foodType == FishType.CopperFish
			   ||i_list[i].m_fish.m_foodType == FishType.SilverFish
			   ||i_list[i].m_fish.m_foodType == FishType.GoldFish)
				continue;
		...
		for (int i = 0; i < i_list.Count; i++)
		{
			if (i_list[i].m_isMatched || i_list[i].GetFishType() == FishType.GoldFish)// || m_iceBlocks[i].rigidbody.velocity.magnitude > 0.5f)
				type = FishType.None;
	...
	{
		float topY = blockDropPos.position.y;
		for(int i = 0; i<m_iceBlocks.Count; i++)
		{
			if(m_iceBlocks[i].transform.position.y > topY)
			{
				topY = m_iceBlocks[i].transform.position.y;
			}
		}
		return topY;
	}
	...
	private void CombineMatchedBlock()
	{
		...
		foreach (List<IceBlock> matchedBlocks in m_matchedBlockQueue)
		{
			...
			int times = matchedBlocks.Count / 3;
			int i = 0;
			while(i < times)
			{

				int insertIndex = m_iceBlocks.IndexOf(matchedBlocks[i]);
				FishType type = matchedBlocks[i].m_fish.m_foodType;
				Vector3 newPosition = matchedBlocks[i].transform.position;// Vector3.zero;
				...
				FishType spawnFishType = matchedBlocks[0].m_fish.m_foodType;
				if( (int)matchedBlocks[0].m_fish.m_foodType <= 2 )
					spawnFishType = (FishType)3;
				else
					spawnFishType = spawnFishType + 1;
				blockScript.SetType(spawnFishType);
			...
			InitBlocks(matchedBlocks.Count - matchedBlocks.Count / 3, firstType, secondType);
